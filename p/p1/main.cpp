...
public:
    Cache(int bits_for_memory, int cache_size/*in bytes*/, int block_size/*in bytes*/, int k_associativity = 2, int victims_cache = 2)
     :memory_size_(bits_for_memory), cache_size_(cache_size), block_size_(block_size), k_way_set_association_(k_associativity), victims_cache_(victims_cache),
    ...
    void push_top_victum_list(Block x)
    {
        ...
        if(index < victims_cache_)
        {
            if(victims_list_[index].available || x == victims_list_[index])
            {
                // std::cout << "found empty / matching block" << std::endl;
                victims_list_[index] = x;
                victims_list_[index].available = false;
                return;
            }
            else
            {
                // std::cout << "writing over index: " << index << std::endl;
                Block temp = victims_list_[index];
                victims_list_[index] = x;
                victims_list_[index].available = false;
                // std::cout << "calling push_top_block_list(temp, set, " << index+1 << ')' << std::endl;
                push_top_victum_list(temp);
            }
        }
    }


    void push_top_block_list(Block x, int set, int index = 0)
    {
        // std::cout << "in push_top_block_list" << std::endl;
        if(index < number_of_cache_lines_/std::pow(2, set_bits_))
        {
            if(blocks_list_[set][index].available || x == blocks_list_[set][index])
            {
                // std::cout << "found empty / matching block" << std::endl;
                blocks_list_[set][index] = x;
                blocks_list_[set][index].available = false;
                return;
            }
            else
            {
                // std::cout << "writing over index: " << index << std::endl;
                Block temp = blocks_list_[set][index];
                blocks_list_[set][index] = x;
                blocks_list_[set][index].available = false;
                // std::cout << "calling push_top_block_list(temp, set, " << index+1 << ')' << std::endl;
                push_top_block_list(temp, set, index+1);
            }
        }
        else
        {
            // std::cout << "pusing to victum_cache" << std::endl;
            Block temp = blocks_list_[set][index-1];
            push_top_victum_list(temp);
        }
    }
